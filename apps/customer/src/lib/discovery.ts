...
import { STORAGE_BUCKET, localizedName } from './listings';
...
export async function getPropertyDetail(id: string): Promise<PropertyDetail | null> {
  const { data, error } = await supabaseClient
    ...
    .select(
      ...
      reviews ( id, overall, cleanliness, accuracy, location, value, checkin, communication, comment_text, created_at, status, author_id, booking_id, property_id, deleted_at, target, published_at, updated_at )
    `,
    )
    .eq('id', id)
    .eq('status', 'approved')
  ...
  // Only show published reviews; newest first.
